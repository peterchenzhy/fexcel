        ...
        _moduleApi: true, getItemsCount: function (f, b) {
            ...
            for (var e = 0; e < l.data.length; e++) {
                if (f) {
                    ...
                    d.push({offset: {x: g.x + l.data.data[e], y: g.y + (b ? g.height : 0)}, value: l.data.xvalues[e]})
                }
            }
            return d
            ...
            if (g.polar || g.spider) {
                for (var d = 0; d < j.polarLabels.length; d++) {
                    ...
                    c.push({offset: {x: f.x, y: f.y}, value: f.value})
                }
                return c
            }
            for (var d = 0; d < j.items.length; d++) {
                if (e) {
                    ...
                    c.push({
                        offset: {x: j.rect.x + j.rect.width, y: j.itemOffsets[j.items[d]].y + j.itemWidth / 2},
                        value: j.items[d]
                    })
                }
            }
